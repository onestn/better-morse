...
<body>
	<div class="wrapper">
		...
		<div class="main-panel">
			...
			<h2 class="ml-4 mt-4 mb-4 text-monospace text-center"> English Word </h2><br>
	...
	analyseWord(words[indexCnt]);
	// # FUNC : String -> Array(MorseCode)
	// 1. word를 받음
	// 2. word를 문자별로 나눔 
	// 3. 나뉜 문자를 모스부호로 변환하여 배열에 저장
	function analyseWord(word) {
		// 파라미터로 받은 값을 하나씩 나누어 배열로 저장
		var splitedWord = word.split('');	
		console.log("파라미터로 받은 단어 : " + splitedWord);
		// 새로운 번역값을 대입하기 위해 초기화
		translatedWord = new Array();
		// words의 길이만큼 반복하여 모스부호 값을 배열에 대입
		for (i in splitedWord) {
			translatedWord.push(getKeyByValue(this.code, splitedWord[i]));
			console.log(splitedWord[i]);
		}
		
		console.log("받은 단어 : " + word + ", 번역된 부호 : " + translatedWord);
		
		// 파라미터로 받은 word를 화면에 출력
		htmlWord.innerText = word;
		htmlCode.innerHTML = "<br>";
		htmlCode.innerText += translatedWord.toString().replaceAll(",", "\n\n");
		...
		if ("key" in evt) {
			...
			// Enter로 사용자의 입력값과 답을 비교한다.
				...
				/* // 다르다면 다시 입력할 수 있도록 
					...
					userAnswer = new Array();
					htmlWord.innerText = "False..";
					// 익명함수 지정
					setTimeout(() => {
						htmlWord.innerText = words[indexCnt];
					}, 1000);
	...
	function checkspace () {
		...
		if (diff > 2*dotlength) {
			if (code[lastchar]) {  
				...
				// userAnswer에 문자 넣기
				userAnswer.push(lastchar);
				// 부호에 해당하는 문자 화면에 출력
				/* showChar.innerText += this.code[lastchar]; */
				space.innerText = "";
			} else if (lastchar) {
				append("*", "userInput");
				queue.add('*');
