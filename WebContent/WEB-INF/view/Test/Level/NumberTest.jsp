...
<head>
	...
	<title>Better Morse (ListenNumber)</title>
...
<script>
	...
	function GenerateQuestion() {
		...
		for (let i = 0; i < 20; i++) {
			let tempNumbers = [];
			for (let i = 0; i < 5; i++) {
				tempNumbers.push(Math.floor(Math.random() * 10));
			}
			totalQuestion.push(tempNumbers);
		}
	...
	window.onload = function() {
		...
		document.getElementById('round').innerText = "Learn Number";
		// 제공하지 않기
		htmlFlash.innerText = totalQuestion[roundIndex].toString().replaceAll(",", " ");
	}
	...
	function ClickSoundBtn() {
		soundMorseCode(toMorse(totalQuestion[roundIndex].toString()));
	...
	function ClickNextBtn() {
		...
		let tempQuestion = totalQuestion[roundIndex].toString().split('');
	...
	function resultLocation() {
		if (resultWrongCharacter != null) {
			alert("제일 많이 틀린 문자입니다. " + "\n\n     [" + resultWrongCharacter.toString().replaceAll(",", " ") + "]\n\n ListenMain화면으로 이동합니다.");
		} else {
			alert("대단한 실력이네요!\n\n ListenMain 화면으로 이동합니다.");
		}
		$.ajax({
		...
		window.location.href = "/Test/TestMain.do";
	}
